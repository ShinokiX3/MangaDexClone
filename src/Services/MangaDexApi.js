...
class MangaDexApi {
	CorsProxy = 'https://justcors.com/tl_54251f9/';
	...
	getMangaCover = async (mangaId) => {
		const mangaImageId = filterSomeAttribute(
			(await fetch(`${this.BaseManga}/${mangaId}/`).then((data) => data.json()))
				?.data?.relationships,
			'cover_art'
		)?.id;
		const coverId = (
	...
	getChapterHash = async (chapterId) => {
		return await fetch(`${this.AtHomeServer}/${chapterId}${this.Setting}`).then(
			(data) => data.json()
		);
	};
	...
	getFilteredData = async (
		...
		const contentRating =
			rating.length > 0
				...
				: '';
		const statusElems =
			status.length > 0 ? status?.map((el) => `&status[]=${el}`).join('') : '';
