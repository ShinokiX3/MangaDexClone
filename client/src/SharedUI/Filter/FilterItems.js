    ...
    return (
        <>
            ...
            <div style={{display: 'flex', flexWrap: 'wrap', alignItems: 'center'}}>
                {items?.map(item => (
                    <FilterItemsViev title={item?.attributes?.name?.en ?? item} id={item?.id} handleClick={handleClick} />
                ))}
                {isSpecific ? 
                    <FilterItemsViev cRef={refAny} title='Any' style={`${values.length === 0 ? `${styles.any}` : ``}`} />
                    :
                    null
                }
            </div>
